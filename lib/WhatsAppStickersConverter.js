...
class WhatsAppStickersConverter {
    ...
    async resizeAndConvert(input, px, toWebp) {
      ...
      if (width !== px || height !== px) {
        if (width > height) {
          image = image.resize(px, Jimp.AUTO);
        } else {
          image = image.resize(Jimp.AUTO, px);
        }

        width = image.getWidth();
        height = image.getHeight();
        const transparent = await new Jimp(px, px);

        if (width > height) {
          image = transparent.composite(image, 0, Math.floor((px - height) / 2));
        } else {
          image = transparent.composite(image, Math.floor((px - width) / 2), 0);
        }
      }
